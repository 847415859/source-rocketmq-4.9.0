...
public class RouteInfoManager {
    ...
    private final HashMap<String/* clusterName */, Set<String/* b rokerName */>> clusterAddrTable;
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                this.lock.writeLock().lockInterruptibly();  // 获取锁
                // 获取 broker 的名称
                ...
                if (null == brokerData) {   // 说明第一次注册，需要放入到 Broker地址缓存中
                    registerFirst = true;
                ...
                while (it.hasNext()) {
                    ...
                    // 如果在原来的地址里面发现和现在的注册的 brokerId 不一样,则说明可能进行了身份的转换(默认 master = 0 )，需要将原有的注册信息移除。
                ...
                // 如果主题信息不为空，并且是 master角色
                if (null != topicConfigWrapper
                    && MixAll.MASTER_ID == brokerId) {
                    if (this.isBrokerTopicConfigChanged(brokerAddr, topicConfigWrapper.getDataVersion())    // 查看主题的配置信息是否改变，通过 dataVersion
                        || registerFirst) {
                ...
                // 如果当前注册的不是 master ,则需要返回当前节点的 集群地址 和 master 的地址
    ...
    private void createAndUpdateQueueData(final String brokerName, final TopicConfig topicConfig) {
        ...
        if (null == queueDataList) {
            ...
            // 判断是是否是新添加的主题的队列，如果是则加入到队列配置信息中
            Iterator<QueueData> it = queueDataList.iterator();
            while (it.hasNext()) {
                QueueData qd = it.next();
                if (qd.getBrokerName().equals(brokerName)) {
                    if (qd.equals(queueData)) { // 不是新队列
                        addNewOne = false;
                    } else {
