...
public class BrokerController {
    ...
    public boolean initialize() throws CloneNotSupportedException {
        // 加载 topic 配置文件 topics.json
        boolean result = this.topicConfigManager.load();
        // 加载消费者偏移量  comsumerOffset.json，订阅组 subscriptionGroup.json ，消费过滤文件 comsumerFilter.json
        ...
        if (result) {
            ...
            this.pullMessageExecutor = new BrokerFixedThreadPoolExecutor(
                this.brokerConfig.getPullMessageThreadPoolNums(),   /* 48 */
                this.brokerConfig.getPullMessageThreadPoolNums(),   /* 48 */
                1000 * 60,
                TimeUnit.MILLISECONDS,
            ...
            this.replyMessageExecutor = new BrokerFixedThreadPoolExecutor(
                this.brokerConfig.getProcessReplyMessageThreadPoolNums(),  /* 48 */
                this.brokerConfig.getProcessReplyMessageThreadPoolNums(),  /* 48 */
                1000 * 60,
                TimeUnit.MILLISECONDS,
            ...
            this.queryMessageExecutor = new BrokerFixedThreadPoolExecutor(
                this.brokerConfig.getQueryMessageThreadPoolNums(),   /* 24 */
                this.brokerConfig.getQueryMessageThreadPoolNums(),   /* 24 */
                1000 * 60,
                TimeUnit.MILLISECONDS,
            ...
            // 设置 Namesr 地址，为后续进行通讯
            if (this.brokerConfig.getNamesrvAddr() != null) {
                this.brokerOuterAPI.updateNameServerAddressList(this.brokerConfig.getNamesrvAddr());
                log.info("Set user specified name server address: {}", this.brokerConfig.getNamesrvAddr());
            // 如果没有设置 NameSrv则，获取地址服务器地址
            ...
            if (!messageStoreConfig.isEnableDLegerCommitLog()) {
                // 如果角色为 Slave ,则获取到集群地址更新，如果是 Master 需要定期的打印主从的差异
    ...
    private void initialTransaction() {
        // 通过SPI加载事务消息服务，如果用户自定义使用用户自定义的事务处理，如果未自定义则使用默认的事务消息处理
        ...
        // 通过SPI加载事务检查监听器，如果用户自定义使用用户自定义的事务检查监听器，如果未自定义则使用默认的事务检查监听器
        // // 通过SPI加载事务消息服务，如果用户自定义使用用户自定义的事务处理，如果未自定义则使用默认的事务消息处理
    ...
    public void registerProcessor() {
        ...
        /**
         * ConsumerManageProcessor  消费管理处理器
         */
        ...
        /**
         * EndTransactionProcessor  事务处理管理器
         */
    ...
    public void start() throws Exception {
        if (this.messageStore != null) {
            this.messageStore.start();  // 开启我消息存储服务
        }
        // 开启远程服务器
        if (this.remotingServer != null) {
        ...
        // Broker对外的API接口，用于于 producer 和 consumer 通讯
        if (this.brokerOuterAPI != null) {
        ...
        // 定期向所有的 NameSrv 注册当前的 broker (默认每隔 30s)
        if (!messageStoreConfig.isEnableDLegerCommitLog()) {
    ...
    private void doRegisterBrokerAll(boolean checkOrderConfig, boolean oneway,
        ...
        List<RegisterBrokerResult> registerBrokerResultList = this.brokerOuterAPI.registerBrokerAll(
            this.brokerConfig.getBrokerClusterName(),       // 集群名
            this.getBrokerAddr(),                           // broker地址
            this.brokerConfig.getBrokerName(),              // broker 名称
            this.brokerConfig.getBrokerId(),                // broker id
            this.getHAServerAddr(),                         // 获取集群地址
            topicConfigWrapper,                             // 所有主题信息
            this.filterServerManager.buildNewFilterServerList(),
            oneway,
            this.brokerConfig.getRegisterBrokerTimeoutMills(),  // 过期时间 60s
            this.brokerConfig.isCompressedRegister());       // 是否是需要压缩
